% Nicola Visentin, 29/11/2024, Politecnico di Milano
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                       %
% Case:   fixed end-state and free end-time; constrained                %                                    
...
u_min=0;        % min thrust [N]
u_max=1e7;      % max thrust [N]

h_i=100;        % initial altitude [m]
h_f=50000;      % final altitude [m]
h_min=0;        % min altitude [m]
h_max=h_f;      % max altitude [m]

v_i=100;        % initial velocity [m/s]
v_f=1000;       % final velocity [m/s]
v_min=0;        % min speed [m/s]
v_max=2000;     % max speed [m/s]

m_i=20000;      % initial mass (shuttle + fuel) [kg]
m_f=10000;      % final mass (shuttle + fuel) [kg]
m_min=5000;     % min mass (shuttle + fuel) [kg]
m_max=m_i;      % max mass (shuttle + fuel) [kg]

gamma_i=pi/3;    % initial angle [rad]
gamma_f=pi/6;    % final angle [rad]
gamma_min=deg2rad(50); % min angle [rad]
gamma_max=pi/2;  % max angle [rad]

x_i=[h_i v_i m_i gamma_i]';
x_f=[h_f v_f m_f gamma_f]';

% weights for the control and the state and the final time
...
tau = 10/(100)^2;   % weight on the final time

P=[p_altitude 0 0 0; 0 p_velocity 0 0; 0 0 p_mass 0; 0 0 0 p_angle];     % Phi=0.5*(x(tf)-xf)'*P*(x(tf)-xf)+tau*(tf-t0)
R=r_control;                                                             % L=0.5*(u'*R*u+x'*Q*x)
Q=[q_altitude 0 0 0; 0 q_velocity 0 0; 0 0 q_mass 0; 0 0 0 q_angle];     % L=0.5*(u'*R*u+x'*Q*x)

nx=size(P,1);   % number of states
nu=size(R,1);   % number of controls
...
phi = @(x,tf) 0.5*(x-x_f)'*P*(x-x_f)+tau*(tf-t0);
...
% define initial guess: z_guess=[x0' u0' x1' u1' ... xN-1' uN-1' xN' tf]'

u_guess=m_i*g*ones(N,nu);                      % choose initial guess for the control: (N x nu) vector
tf_guess=70;    % choose initial guess for the final time
z_guess=assign_z0(nx,nu,N,x_i,u_guess,tf_guess,xp);
...
z_lb=AssignZbound([h_min v_min m_min gamma_min],u_min,0,param);
z_ub=AssignZbound([h_max v_max m_max gamma_max],u_max,inf,param);
...
fprintf('Optimal final time: tf_opt = %g s\n\n',z(end))
...
for ii=0:N
    x_opt(ii+1,:)=z((1+ii*(nu+nx)):(nx+ii*(nu+nx)));
end    
for ii=0:N-1
    u_opt(ii+1,:)=z((1+nx+ii*(nu+nx)):(nx+nu+ii*(nu+nx)));    
end
...
subplot(2,2,4)
hold on
plot(t_x,rad2deg(x_opt(:,4)),'m')
plot(tf_opt,rad2deg(x_f(4)),'mx')
grid on
box on
xlabel('t [s]',Interpreter='latex')
...
save('optimal_u.mat','u_opt','tf_opt')