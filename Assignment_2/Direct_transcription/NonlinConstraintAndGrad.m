...
if nargout > 2
    ...
    DCeq = zeros((N+1)*nx,N*(nu+nx) + nx); % gradient of a vector
    DCeq(1:nx,1:nx) = - eye(nx); 
    for ii = 1:N
        DCeq((1 + nx +(ii - 1)*nx):((nx + ii*nx)),(1 +(ii - 1)*(nx+nu)):((ii - 1)*(nx+nu) + nx)) = ...
             eye(nx) + h*dfdx(x(:,ii),u(:,ii));
        DCeq((1 + nx +(ii - 1)*nx):((nx + ii*nx)),(1 +(ii)*(nx+nu)):((ii)*(nx+nu) + nx)) = ...
             - eye(nx);
        DCeq((1 + nx +(ii - 1)*nx):((nx + ii*nx)),(1 +(ii - 1)*(nx+nu) + nx):((ii - 1)*(nx+nu) + nx + nu)) = ...
             + h*dfdu(x(:,ii),u(:,ii));
    end
    DCeq = DCeq.';
