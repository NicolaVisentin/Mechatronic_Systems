...
r_u1 = 0.01/(m_i*g)^2;           % weight for the control action u1 (integral cost)
r_u2 = 0.001/(pi/2)^2;            % weight for the control action u2 (integral cost)
...
R=[r_u1 0; 0 r_u2];                                                      % L=0.5*(u'*R*u+x'*Q*x)
...
tau_i=[1e9 1e-3];        % speed of control adjustment (exponentially reduces during iterations); two parameters for the two controls
tau_f=[1e-2 1e-7];
...
while ii<ii_max && dHdu_norm>tol
    ...
    tau(1)=(tau_i(1)-tau_f(1))*exp(-5*(ii+1)/ii_max)+tau_f(1);
    tau(2)=(tau_i(2)-tau_f(2))*exp(-5*(ii+1)/ii_max)+tau_f(2);
    ...
    if animation==1 && (mod(ii,N_anim)==0 || ii==-1)
        ...
        set(pl_u1,'XData',t_u,'YData',u(:,1))
        set(pl_u2,'XData',t_u,'YData',rad2deg(u(:,2)))
