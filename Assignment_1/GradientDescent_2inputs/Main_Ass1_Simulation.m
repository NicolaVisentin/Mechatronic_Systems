...
% u=[m_i*g*(ones(N,1)+evolution) zeros(N,1)];
...
u_anim=interp1(t_u,u(:,1),t_anim);
beta_anim=interp1(t_u,u(:,2),t_anim);
...
len_fl=3*u_anim*len_sh/max(u_anim);             % "flame" length: visualisation of u
...
title('Inlcination')
xlim([t0 tf])
ylim(rad2deg([min(gamma_anim) max(gamma_anim)]))
...
yyaxis right
pl_beta=plot(nan,nan,'b');
ylabel('$\beta$ [deg]','Interpreter','latex')
ylim(rad2deg([min(beta_anim) max(beta_anim)]))

grid on
box on
xlabel('$t$ [s]','Interpreter','latex')
title('Control')
legend({'$u(t)$','$\beta(t)$'},'Interpreter','latex','Location','best')
ax=gca;
ax.YAxis(1).Color='r';
ax.YAxis(2).Color='b';
xlim([t0 tf])
...
for ii=2:N_anim
    ...
    xu=xb-len_fl(ii)*cos(gamma_anim(ii)-beta_anim(ii));
    yu=yb-len_fl(ii)*sin(gamma_anim(ii)-beta_anim(ii));
    ...
    set(pl_beta,'XData',t_anim(1:ii-1),'YData',rad2deg(beta_anim(1:ii-1)))
